...
- 首次进入提供本地工作区初始化，模拟注册、额度初始化和目标配置。
...
- 生产环境使用 Node API 保存项目、知识库和构建运行记录，浏览器离线或 API 不可用时降级到 `localStorage`。
- 新增实时数据页，展示 BuilderOS API、Build Engine、RAG Engine、服务端进程、持久化文件和最近构建记录。
